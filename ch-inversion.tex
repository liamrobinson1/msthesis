...
where $A$ and $B$ are two events and $P(B) \neq 0$. In the context of shape inversion, we seek to find the shape $A$ that best explains the light curve $B$. To evaluate the probability of a given shape mesh estimate $\mathcal{M}_i$ based on light curve data $I_{1:i}$, the quantities $P(\mathcal{M}_i)$ and $P(I_{1:i})$ are required. The probability of a given mesh estimate $P(\mathcal{M}_i)$ is difficult to evaluate, as it is not clear how to define a prior distribution over the space of all possible meshes. Even if a discrete distribution of object shapes could be built from known object types in the catalog, the process of measuring the statistical distance of a given shape from the elements of that distribution is intractable. The probability of the light curve $P(I_{1:i})$ is also difficult to evaluate, as it is not clear how to define a prior distribution over the space of all possible light curves. Such a distribution would have to account for all possible object shapes, attitude profiles, orbits, observer conditions, observer configurations, and observation times --- a clearly impractical calculation. As a result, it is impractical to perform a full Bayesian update on a given shape estimate.
