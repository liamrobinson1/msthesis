...
By Eq \ref{eq:egi_deg}, flat areas of the object are mapped to point masses in the EGI, motivating a simpler definition of the EGI for discrete surfaces. The discrete EGI $\vctr{E} \in \mathbb{R}^{m \times 3}$ is composed of $m$ unit vectors $\hat{n}$ each scaled by a nonnegative areea $a \in \mathbb{R}$, where $a_i \geq 0 \forall i$ \cite{little1983}.

\begin{equation}
  \vctr{E}_i = a_i \hat{n}_i
\end{equation}

In the context of shape inversion, the $m$ vectors $\hat{n}$ should be a relatively uniform tessellation of the unit sphere. A convex polytope can be uniquely represented by an EGI of facet normal vectors scaled by each facet's area. The set of normal vectors in an EGI is denoted $\mathcal{N}$ with the set of areas denoted $\mathcal{A}$. The vector of facet areas is denoted $\vctr{a} \in \mathbb{R}^{m \times 1}$. The norm of the EGI is notated $\| \vctr{E} \| = \vctr{a}$ with the `size' of the EGI $\|\vctr{E}\| = m$.
...
\begin{equation} \label{eq:egi_validation}
  \vctr{E}_{\textrm{closed}} = \vctr{E}_{\textrm{open}} - \sum_{i=1}^m a_i \hat{n}_i .
\end{equation}
...
\begin{equation} \label{eq:closure_error}
  \vctr{e}_{EGI} = -\sum_{i=1}^{m} a_i \hat{n}_i.
\end{equation}
