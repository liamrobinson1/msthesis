...
% \begin{figure}[!htb]
%   \centering
%   \includegraphics[width=\figmed]{brightness_iso.png}
%   \caption{Brightness isosurfaces for a cube with $C_d=0.5$, $C_s=0.5$, $n=5$ at $\phi=45^\circ$ and normalized irradiance value $\hat{I} = 0.73$}
...
Objects with symmetries in their albedo-area are also ambiguous in the shape inversion problem.

\subsubsection{Observation Geometry}

If the directions of the Sun and observer in the body frame of the object are flipped --- thereby mirroring the object about the plane containing the Sun-object-observer bisector vector --- the light curve will be identical \cite{burton2023attitude}. This follows from the reciprocity of light reflection as modeled by the BRDF as well as the reciprocity of the shadows \cite{burton2023attitude}. Upon mirroring the object as described, areas which were previously Sun-shadowed become observer-shadowed and vice versa, preserving the total amount of shadowing and therefore the overall light curve. This creates an ambiguity in the object orientation as it yields two equally plausible solutions for the object's attitude profile \cite{burton2023attitude}. This orientation ambiguity produces ambiguity in the object's shape as well: the observed light curve may correspond to either the flipped or ``nominal'' orientation of the object.
