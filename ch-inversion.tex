...
Bayes' theorem for the light curve inversion shape inversion problem is stated \cite{mahler2014}:

\begin{equation}
  P(M^\ell \vert I_{1:i}) = \frac{P(I_{1:i} \vert M^\ell)P(M^\ell)}{P(I_{1:i})},
\end{equation}

where $M^\ell$ is a particular shape hypothesis and $I_{1:i}$ all the light curve measurements up to time $i$. 
