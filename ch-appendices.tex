...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \State $\boldsymbol{\ell} \in \mathbb{S}^2$ \Comment{Unit vector from object origin towards Sun}
    \State $\vctr{o} \in \mathbb{S}^2$ \Comment{Unit vector from object origin towards observer}
    \State $\vctr{n}\in \mathbb{S}^2$ \Comment{Outward-pointing surface normal vector at pixel coordinates}
    ...
    \State $D_\mathrm{obs} \gets \left( \boldsymbol{\ell} - \vctr{o} \right) \cdot \boldsymbol{\ell}$ \Comment{Closest pixel depth in the Observer direction}
    ...
    \EndIf
    \If{$\left(\vctr{n}\cdot L\right) > 0 \textrm{ and } \left(\vctr{n}\cdot \vctr{o}\right) > 0$}
      \State $f_r(\vctr{x}, \boldsymbol{\ell},\vctr{o}) = 0$ \Comment{Pixel cannot be both observed and illuminated}
    \Else 
      \State $f_r(\vctr{x}, \boldsymbol{\ell},\vctr{o}) = \mathrm{Phong}(\boldsymbol{\ell}, \vctr{o}, \vctr{n}, C_d, C_s, n)$ \Comment{The pixel is shaded with the BRDF}
    \EndIf
    \State $\mathrm{IM}(x, y) = f_r(\vctr{x}, \boldsymbol{\ell},\vctr{o}) \left(\vctr{n}\cdot \boldsymbol{\ell}\right) $ \Comment{Image pixel value}
  \end{algorithmic}
\end{algorithm}
