...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \State $\vctr{\ell} \in \mathbb{S}^2$ \Comment{Unit vector from object origin towards Sun}
    ...
    \State $D_\mathrm{obs} \gets \left( \vctr{\ell} - \vctr{o} \right) \cdot \vctr{\ell}$ \Comment{Closest pixel depth in the Observer direction}
    ...
    \If{$\left(\vctr{n}\cdot L\right) > 0 \textrm{ and } \left(\vctr{n}\cdot \vctr{o}\right) > 0$}
      \State $f_r(\vctr{x}, \vctr{\ell},\vctr{o}) = 0$ \Comment{Pixel cannot be both observed and illuminated}
    \Else 
      \State $f_r(\vctr{x}, \vctr{\ell},\vctr{o}) = \mathrm{Phong}(\vctr{\ell}, \vctr{o}, \vctr{n}, C_d, C_s, n)$ \Comment{The pixel is shaded with the BRDF}
    \EndIf
    \State $\mathrm{IM}(x, y) = f_r(\vctr{x}, \vctr{\ell},\vctr{o}) \left(\vctr{n}\cdot \vctr{\ell}\right) $ \Comment{Image pixel value}
  \end{algorithmic}
\end{algorithm}
